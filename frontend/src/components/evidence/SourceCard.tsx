...
export function SourceCard({ title, type, url, snippet, pages, chunks, grounding = 'high', className }: SourceCardProps) {
  const isWeb = type === 'web';
  const hostname = isWeb && url ? new URL(url).hostname.replace('www.', '') : '';
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0 flex-1 space-y-1">
          ...
          <div className="flex flex-wrap items-center gap-x-3 gap-y-0.5">
            ...
            <span className={cn('rounded-full px-1.5 py-0.5 text-[9px] font-medium uppercase tracking-wider', groundingConfig[grounding].class)}>
              {groundingConfig[grounding].label}
            </span>
          </div>
