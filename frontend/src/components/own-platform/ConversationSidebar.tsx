        ...
        {loading ? (
          ...
          <div className="space-y-1 px-1 py-1">
            {pinned.length > 0 && !searchQuery && (
              <>
                <div className="px-2 py-1.5 text-caption font-medium text-muted-foreground/40 uppercase tracking-wider flex items-center gap-2">
            ...
            {grouped.map(group => (
              <div key={group.label}>
                <div className="px-2 py-1.5 text-caption font-medium text-muted-foreground/40 uppercase tracking-wider">
                  {group.label}
                </div>
...
function SessionRow({
  ...
  return (
    <div
      className={cn(
        'group relative flex items-center gap-2 rounded-lg px-3 py-2 transition-all cursor-pointer',
        isActive ? 'bg-primary/[0.06]' : 'hover:bg-hover/50',
      )}
