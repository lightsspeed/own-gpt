...
class Settings(BaseSettings):
    ...
    # Groq credentials — required ONLY when LLM_PROVIDER=groq. Groq exposes
    # an OpenAI-compatible API (base URL + ChatOpenAI), so no extra SDK.
    GROQ_API_KEY: str = ""
    GROQ_BASE_URL: str = "https://api.groq.com/openai/v1"
    GROQ_MODEL: str = "llama-3.3-70b-versatile"

    # LLM provider — local development defaults to Ollama (no paid API).
    # "ollama" | "openai" | "groq" | "gemini". There is NO automatic fallback
    # between providers: the configured provider is the only one ever constructed.
    # If Ollama is unreachable, requests fail clearly — OpenAI/Groq/Gemini are
    # never silently used.
    ...
    # Gemini credentials — required ONLY when LLM_PROVIDER=gemini.
    # Uses the official google-genai SDK via langchain-google-genai.
    # Never hardcoded; key is read exclusively from environment/.env.
    GEMINI_API_KEY: str = ""
    ...
    # Providers: "gemini" | "openai" | "none". The "ollama" branch exists in
    ...
    # memory schema — memory embeddings must remain "gemini", "openai" or "none" until
    # a dimension migration is separately decided. Never set this to "ollama".
    MEMORY_EMBEDDING_PROVIDER: str = "gemini"     # "gemini" | "openai" | "none"
    MEMORY_EMBEDDING_MODEL: str = "gemini-embedding-2"
    # Knowledge Base (RAG) embeddings — configuration-driven provider selection.
    # Providers: "ollama" (default, nomic-embed-text) | "openai" | "none".
    # Isolated from Memory V2 embeddings (which remain pinned to MEMORY_EMBEDDING_*).
    KB_EMBEDDING_PROVIDER: str = "ollama"
    KB_EMBEDDING_MODEL: str = "nomic-embed-text"

    # Ollama embedding model (used by OllamaEmbeddingProvider / build_kb_embeddings).
